...
impl Register<LORA> for PPMCorrection{}

impl ReadableRegister<LORA> for PPMCorrection{
    const ADDRESS: u8 = 39;
    const SIZE: usize = 1;
}
impl<'x> TryFrom<&'x[u8]> for PPMCorrection {
...
impl Into<u8> for PPMCorrection {
    fn into(self) -> u8 {
        (self.0 & 255) >> 0
    }
}

impl WritableRegister<LORA> for PPMCorrection{
    const ADDRESS: u8 = 39;
    const SIZE: usize = 1;
}
impl TryWriteInto<[u8], usize> for PPMCorrection {
    type Error = SX1278Error;
