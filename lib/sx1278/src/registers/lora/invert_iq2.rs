...
/// Controls IQ Inversion
#[derive(Clone, Copy)]
pub struct InvertIQ2 (u8);

impl Register<LORA> for InvertIQ2{}

impl ReadableRegister<LORA> for InvertIQ2{
    const ADDRESS: u8 = 59;
    const SIZE: usize = 1;
}
impl<'x> TryFrom<&'x[u8]> for InvertIQ2 {
...
impl Into<u8> for InvertIQ2 {
    fn into(self) -> u8 {
        (self.0 & 255) >> 0
    }
}

impl WritableRegister<LORA> for InvertIQ2{
    const ADDRESS: u8 = 59;
    const SIZE: usize = 1;
}
impl TryWriteInto<[u8], usize> for InvertIQ2 {
    type Error = SX1278Error;
