...
/// RW Base address in FIFO for RX operations
#[derive(Clone, Copy)]
pub struct FifoRxBaseAddr (u8);

impl Register<LORA> for FifoRxBaseAddr{}

impl ReadableRegister<LORA> for FifoRxBaseAddr{
    const ADDRESS: u8 = 15;
    const SIZE: usize = 1;
}
impl<'x> TryFrom<&'x[u8]> for FifoRxBaseAddr {
...
impl Into<u8> for FifoRxBaseAddr {
    fn into(self) -> u8 {
        (self.0 & 255) >> 0
    }
}

impl WritableRegister<LORA> for FifoRxBaseAddr{
    const ADDRESS: u8 = 15;
    const SIZE: usize = 1;
}
impl TryWriteInto<[u8], usize> for FifoRxBaseAddr {
    type Error = SX1278Error;
