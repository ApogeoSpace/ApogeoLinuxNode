...
impl Register<LORA> for SyncWord{}

impl ReadableRegister<LORA> for SyncWord{
    const ADDRESS: u8 = 57;
    const SIZE: usize = 1;
}
impl<'x> TryFrom<&'x[u8]> for SyncWord {
...
impl Into<u8> for SyncWord {
    fn into(self) -> u8 {
        (self.0 & 255) >> 0
    }
}

impl WritableRegister<LORA> for SyncWord{
    const ADDRESS: u8 = 57;
    const SIZE: usize = 1;
}
impl TryWriteInto<[u8], usize> for SyncWord {
    type Error = SX1278Error;
