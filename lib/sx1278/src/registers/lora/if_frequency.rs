...
/// Errata IFFreq2/3
#[derive(Clone, Copy)]
pub struct IfFrequency (u16);

impl Register<LORA> for IfFrequency{}

impl ReadableRegister<LORA> for IfFrequency{
    const ADDRESS: u8 = 47;
    const SIZE: usize = 2;
}
impl<'x> TryFrom<&'x[u8]> for IfFrequency {
...
impl Into<u16> for IfFrequency {
    fn into(self) -> u16 {
        (self.0 & 65535) >> 0
    }
}

impl WritableRegister<LORA> for IfFrequency{
    const ADDRESS: u8 = 47;
    const SIZE: usize = 2;
}
impl TryWriteInto<[u8], usize> for IfFrequency {
    type Error = SX1278Error;
