...
impl Register<LORA> for PayloadLength{}

impl ReadableRegister<LORA> for PayloadLength{
    const ADDRESS: u8 = 34;
    const SIZE: usize = 1;
}
impl<'x> TryFrom<&'x[u8]> for PayloadLength {
...
impl Into<u8> for PayloadLength {
    fn into(self) -> u8 {
        (self.0 & 255) >> 0
    }
}

impl WritableRegister<LORA> for PayloadLength{
    const ADDRESS: u8 = 34;
    const SIZE: usize = 1;
}
impl TryWriteInto<[u8], usize> for PayloadLength {
    type Error = SX1278Error;
