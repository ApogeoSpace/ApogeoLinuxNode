...
/// Timer 2 coefficient
#[derive(Clone, Copy)]
pub struct Timer2Coef (u8);

impl Register<FSK> for Timer2Coef{}

impl ReadableRegister<FSK> for Timer2Coef{
    const ADDRESS: u8 = 58;
    const SIZE: usize = 1;
}
impl<'x> TryFrom<&'x[u8]> for Timer2Coef {
...
impl Into<u8> for Timer2Coef {
    fn into(self) -> u8 {
        (self.0 & 255) >> 0
    }
}

impl WritableRegister<FSK> for Timer2Coef{
    const ADDRESS: u8 = 58;
    const SIZE: usize = 1;
}
impl TryWriteInto<[u8], usize> for Timer2Coef {
    type Error = SX1278Error;
