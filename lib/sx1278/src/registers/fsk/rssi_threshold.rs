...
/// RSSI threshold
#[derive(Clone, Copy)]
pub struct RssiThreshold (u8);

impl Register<FSK> for RssiThreshold{}

impl ReadableRegister<FSK> for RssiThreshold{
    const ADDRESS: u8 = 16;
    const SIZE: usize = 1;
}
impl<'x> TryFrom<&'x[u8]> for RssiThreshold {
...
impl Into<u8> for RssiThreshold {
    fn into(self) -> u8 {
        (self.0 & 255) >> 0
    }
}

impl WritableRegister<FSK> for RssiThreshold{
    const ADDRESS: u8 = 16;
    const SIZE: usize = 1;
}
impl TryWriteInto<[u8], usize> for RssiThreshold {
    type Error = SX1278Error;
...
impl From<u8> for RssiThreshold {
    fn from(v: u8) -> Self {
        Self(((v as u8) << 0) & 255)
    }
}
