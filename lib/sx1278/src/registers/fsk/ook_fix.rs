...
/// Fixed threshold for OOK
#[derive(Clone, Copy)]
pub struct OokFix (u8);

impl Register<FSK> for OokFix{}

impl ReadableRegister<FSK> for OokFix{
    const ADDRESS: u8 = 21;
    const SIZE: usize = 1;
}
impl<'x> TryFrom<&'x[u8]> for OokFix {
...
impl Into<u8> for OokFix {
    fn into(self) -> u8 {
        (self.0 & 255) >> 0
    }
}

impl WritableRegister<FSK> for OokFix{
    const ADDRESS: u8 = 21;
    const SIZE: usize = 1;
}
impl TryWriteInto<[u8], usize> for OokFix {
    type Error = SX1278Error;
