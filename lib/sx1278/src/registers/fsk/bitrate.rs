...
/// Bit rate configuration. BR = F_{xosc} / (VAL + (1/16 * BRFrac))
#[derive(Clone, Copy)]
pub struct Bitrate (u16);

impl Register<FSK> for Bitrate{}

impl ReadableRegister<FSK> for Bitrate{
    const ADDRESS: u8 = 2;
    const SIZE: usize = 2;
}
impl<'x> TryFrom<&'x[u8]> for Bitrate {
...
impl Into<u16> for Bitrate {
    fn into(self) -> u16 {
        (self.0 & 65535) >> 0
    }
}

impl WritableRegister<FSK> for Bitrate{
    const ADDRESS: u8 = 2;
    const SIZE: usize = 2;
}
impl TryWriteInto<[u8], usize> for Bitrate {
    type Error = SX1278Error;
