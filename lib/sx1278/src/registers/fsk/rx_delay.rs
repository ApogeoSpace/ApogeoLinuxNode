...
impl Register<FSK> for RxDelay{}

impl ReadableRegister<FSK> for RxDelay{
    const ADDRESS: u8 = 35;
    const SIZE: usize = 1;
}
impl<'x> TryFrom<&'x[u8]> for RxDelay {
...
impl Into<u8> for RxDelay {
    fn into(self) -> u8 {
        (self.0 & 255) >> 0
    }
}

impl WritableRegister<FSK> for RxDelay{
    const ADDRESS: u8 = 35;
    const SIZE: usize = 1;
}
impl TryWriteInto<[u8], usize> for RxDelay {
    type Error = SX1278Error;
