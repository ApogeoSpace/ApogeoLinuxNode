...
impl Register<FSK> for BroadcastAdrs{}

impl ReadableRegister<FSK> for BroadcastAdrs{
    const ADDRESS: u8 = 52;
    const SIZE: usize = 1;
}
impl<'x> TryFrom<&'x[u8]> for BroadcastAdrs {
...
impl Into<u8> for BroadcastAdrs {
    fn into(self) -> u8 {
        (self.0 & 255) >> 0
    }
}

impl WritableRegister<FSK> for BroadcastAdrs{
    const ADDRESS: u8 = 52;
    const SIZE: usize = 1;
}
impl TryWriteInto<[u8], usize> for BroadcastAdrs {
    type Error = SX1278Error;
