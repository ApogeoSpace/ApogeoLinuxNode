...
/// Timer 1 coefficient
#[derive(Clone, Copy)]
pub struct Timer1Coef (u8);

impl Register<FSK> for Timer1Coef{}

impl ReadableRegister<FSK> for Timer1Coef{
    const ADDRESS: u8 = 57;
    const SIZE: usize = 1;
}
impl<'x> TryFrom<&'x[u8]> for Timer1Coef {
...
impl Into<u8> for Timer1Coef {
    fn into(self) -> u8 {
        (self.0 & 255) >> 0
    }
}

impl WritableRegister<FSK> for Timer1Coef{
    const ADDRESS: u8 = 57;
    const SIZE: usize = 1;
}
impl TryWriteInto<[u8], usize> for Timer1Coef {
    type Error = SX1278Error;
