...
impl Register<FSK> for DioMapping{}

impl ReadableRegister<FSK> for DioMapping{
    const ADDRESS: u8 = 64;
    const SIZE: usize = 2;
}
...
impl Into<u16> for DioMapping {
    fn into(self) -> u16 {
        (self.0 & 65535) >> 0
    }
}

impl WritableRegister<FSK> for DioMapping{
    const ADDRESS: u8 = 64;
    const SIZE: usize = 2;
}
