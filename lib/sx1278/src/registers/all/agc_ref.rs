...
impl<MODE: SX1278Mode> Register<MODE> for AgcRef{}

impl<MODE: SX1278Mode> ReadableRegister<MODE> for AgcRef{
    const ADDRESS: u8 = 97;
    const SIZE: usize = 1;
}
impl<'x> TryFrom<&'x[u8]> for AgcRef {
...
impl Into<u8> for AgcRef {
    fn into(self) -> u8 {
        (self.0 & 63) >> 0
    }
}

impl<MODE: SX1278Mode> WritableRegister<MODE> for AgcRef{
    const ADDRESS: u8 = 97;
    const SIZE: usize = 1;
}
impl TryWriteInto<[u8], usize> for AgcRef {
    type Error = SX1278Error;
